...
public partial class Students_ManageSchoolFees : System.Web.UI.Page
{
    ...
    public static string BindAcademicYearMonth(string regNo, string academicId, string editPrm, string delPrm, string btype)
    {
        ...
        DataSet dsManageFees_SeparateBill = new DataSet();
        ...
        //Check whether ICStudent or not
        string sqlCheckICstr = "select IsICStudent from s_studentinfo where RegNo= '" + regNo + "'"; 
        //string IsIcTypeStudent = utl.ExecuteScalar(sqlCheckICstr);
        string IsIcTypeStudent = utl.ExecuteScalar(sqlCheckICstr).Trim();

        HttpContext.Current.Session["Isactive"] = Isactive.ToString();
        string query = "";
        SqlCommand cmd = new SqlCommand();
        SqlCommand cmd_IC = new SqlCommand();
        ...
        if (btype == "Single")
        {
            ...
            SqlDataAdapter daManageFees_SeparateBill = new SqlDataAdapter();
            if (IsIcTypeStudent == "Y")
            {
                if (Isactive == "True")
                {
                    cmd_IC = new SqlCommand("sp_ManageFee_SeparateBill");
                }

                cmd_IC.CommandType = CommandType.StoredProcedure;
                cmd_IC.Parameters.AddWithValue("@regno", regNo);
                cmd_IC.Parameters.AddWithValue("@AcademicId", academicId);
                cmd_IC.Parameters.Add("@FeesCatId", SqlDbType.Int).Direction = ParameterDirection.Output;
                cmd_IC.Parameters.Add("@FeesMonthName", SqlDbType.VarChar, 20).Direction = ParameterDirection.Output;
                cmd_IC.Parameters.Add("@FeesTotalAmt", SqlDbType.Decimal, 18).Direction = ParameterDirection.Output;
                cmd_IC.Parameters["@FeesTotalAmt"].Precision = 18;
                cmd_IC.Parameters["@FeesTotalAmt"].Scale = 2;
                cmd_IC.Parameters.Add("@FeesMonth", SqlDbType.VarChar, 20).Direction = ParameterDirection.Output;
                cmd_IC.Parameters.Add("@FeesType", SqlDbType.VarChar, 20).Direction = ParameterDirection.Output;
                cmd_IC.Parameters.Add("@isTerm", SqlDbType.Int).Direction = ParameterDirection.Output;
                conn.Open();
                cmd_IC.Connection = conn;

                daManageFees_SeparateBill.SelectCommand = cmd_IC;

                daManageFees_SeparateBill.Fill(dsManageFees_SeparateBill);
                conn.Close();
            }
                ...
                if (dsManageFees != null && dsManageFees.Tables.Count > 0)
                {
                    if (IsIcTypeStudent == "Y")
                    {
                        firstContent = GetFirstContentwithIC(dsManageFees, dsManageFees_SeparateBill,cmd, cmd_IC, regNo, academicId, editPrm, delPrm);
                    }
                   else
                    {
    ...
    public static string GetFirstContentwithIC(DataSet dsManageSchoolFees, DataSet dsManageFees_SeparateBill, SqlCommand cmd, SqlCommand cmd_IC,string regno, string academicId, string editPrm, string delPrm)
    {
        StringBuilder divFirstContent = new StringBuilder();
        StringBuilder divFirstContent_IC = new StringBuilder();
        string strOptions = string.Empty;
        string _Billno = string.Empty;
        string _BillDate = string.Empty;
        string _BillUser = string.Empty;
        ...
        string strOptions_IC = string.Empty;
        string _Billno_IC = string.Empty;
        string _BillDate_IC = string.Empty;
        string _BillUser_IC = string.Empty;
        string feesCatHeadId_IC = string.Empty;
        string feesHeadAmt_IC = string.Empty;
        string feesCatId_IC = string.Empty;
        string feesMonthName_IC = string.Empty;
        string feestotalAmount_IC = string.Empty;
        ...
        if (cmd_IC.Parameters["@FeesCatId"] != null)
            feesCatId_IC = cmd_IC.Parameters["@FeesCatId"].Value.ToString();

        if (cmd_IC.Parameters["@FeesMonthName"] != null)
            feesMonthName_IC = cmd_IC.Parameters["@FeesMonthName"].Value.ToString();

        if (cmd_IC.Parameters["@FeesTotalAmt"] != null)
            feestotalAmount_IC = cmd_IC.Parameters["@FeesTotalAmt"].Value.ToString();

        if (dsManageFees_SeparateBill.Tables.Count > 4 && dsManageFees_SeparateBill.Tables[3].Rows.Count > 0)
        {
            for (int k = 0; k < dsManageFees_SeparateBill.Tables[3].Rows.Count; k++)
            {

                feesCatHeadId_IC += dsManageFees_SeparateBill.Tables[3].Rows[k]["feescatheadid"].ToString() + "|";
                feesHeadAmt_IC += "0.00|";
            }
        }


        if (dsManageFees_SeparateBill.Tables[0].Rows.Count > 0)
        {
            divFirstContent.Append(" <table class='data display datatable' id='example'><thead><tr><th width='20%' class='sorting_mod center' >Months</th><th width='20% center' class='sorting_mod' >Receipt/Bill No</th><th width='16%' class='sorting_mod center' >Date</th><th width='18%' class='sorting_mod center' >Received By</th><th width='30% center' class='sorting_mod' >Option</th></tr></thead>");
            for (int i = 0; i < dsManageFees_SeparateBill.Tables[0].Rows.Count; i++)
            {


                _Billno_IC = "<td class='center'>-</td>";
                _BillDate_IC = "<td class='center'>-</td>";
                _BillUser_IC = "<td class='center'>-</td>";



                if (dsManageFees_SeparateBill.Tables[0].Rows[i]["monthnum"].ToString() == cmd.Parameters["@FeesMonth"].Value.ToString() && cmd.Parameters["@FeesType"].Value.ToString().Trim().ToUpper() == "ACADEMIC")
                    //  strOptions = "<table cellpadding='10' cellspacing='10' > <tr><td><a href='javascript:CreateBill(\"" + regno + "\",\"" + academicId + "\");'> Create </a> </td><td> <a href='javascript:IgnoreBill(\"" + regno + "\",\"" + academicId + "\");'>  Ignore </a> </td></tr></table>";
                    strOptions_IC = "<table cellpadding='10' cellspacing='10' > <tr><td><a href='javascript:CreateBill();' class='creat-link'> <img src='../img/creat.png' alt='' width='22' height='22' align='absmiddle' />  Create </a> </td><td> <a href='javascript:IgnoreBill(\"" + regno + "\",\"" + academicId + "\",\"" + feesCatHeadId + "\",\"" + feesHeadAmt + "\",\"" + feesCatId + "\",\"" + feesMonthName + "\",\"0.00\");'  class='creat-link' > <img src='../img/ignor.jpg' alt='' width='22' height='22' align='absmiddle' />   Ignore </a> </td></tr></table>";
                else
                    strOptions_IC = "<table cellpadding='10' cellspacing='10' > <tr><td> - </td><td> - </td></tr></table>";
                if (dsManageFees_SeparateBill.Tables[1].Rows.Count > 0)
                {

                    DataRow[] drPaidFees = dsManageFees_SeparateBill.Tables[1].Select("billmonth='" + dsManageFees_SeparateBill.Tables[0].Rows[i]["monthname"].ToString().ToUpper().Trim() + "' and feescatcode<>'F' ");

                    if (drPaidFees.Length > 1)
                    {
                        _Billno_IC = "<td class='center'>" + drPaidFees[1]["billno"].ToString() + "</td>";
                        _BillDate_IC = "<td class='center'>" + drPaidFees[1]["billdate"].ToString() + "</td>";
                        _BillUser_IC = "<td class='center'>" + drPaidFees[1]["StaffshortName"].ToString() + "</td>";
                        strOptions_IC = "<table cellpadding='10' cellspacing='10' > <tr><td> <a href='javascript:ViewBill(\"" + drPaidFees[1]["billid"].ToString() + "\");'  class='creat-link'>  <img src='../img/creat.png' alt='' width='22' height='22' align='absmiddle' />  View </a> </td>";

                        if (HttpContext.Current.Session["Isactive"] != null && HttpContext.Current.Session["Isactive"].ToString() != "" && HttpContext.Current.Session["Isactive"].ToString().ToLower() != "false")
                        {
                            if (delPrm == "true" && HttpContext.Current.Session["Isactive"].ToString().ToLower() == "true")
                            {
                                strOptions_IC += " <td> <a href='javascript:DeleteBill(\"" + drPaidFees[1]["billid"].ToString() + "\");'  class='creat-link' >  <img src='../img/delete.png' alt='' align='absmiddle' />  Delete </a><td>";
                            }
                            if (editPrm == "true" && HttpContext.Current.Session["Isactive"].ToString().ToLower() == "true")
                            {
                                strOptions_IC += " <td> <a href='javascript:PrintBill(\"" + drPaidFees[1]["billid"].ToString() + "\");'  class='creat-link' >  <img src='../img/print.png' alt='' width='22' height='22' align='absmiddle' />  Print </a> </td>";
                            }
                        }
                        else
                        {
                        ...
                        strOptions_IC += " </tr>";
                        if (dsManageFees_SeparateBill.Tables[0].Rows[i]["monthnum"].ToString() == cmd.Parameters["@FeesMonth"].Value.ToString() && cmd.Parameters["@FeesType"].Value.ToString().Trim().ToUpper() == "ACADEMIC")
                            strOptions_IC += "<tr><td><a href='javascript:updateBill(\"" + drPaidFees[1]["billid"].ToString() + "\");' class='creat-link'> <img src='../img/creat.png' alt='' width='22' height='22' align='absmiddle' />  Update Bill </a> </td><td> <a href='javascript:IgnoreExistingBill(\"" + drPaidFees[1]["billid"].ToString() + "\",\"" + academicId + "\",\"" + feesCatHeadId + "\",\"" + feesHeadAmt + "\",\"" + feesCatId + "\",\"" + feesMonthName + "\",\"0.00\");'  class='creat-link' > <img src='../img/ignor.jpg' alt='' width='22' height='22' align='absmiddle' />   Ignore </a> </td></tr>";
                ...
                if (cmd.Parameters["@isTerm"] != null && cmd.Parameters["@isTerm"].Value.ToString() == "1")
                {
                    DataRow[] monthtoterm = dsManageFees_SeparateBill.Tables[dsManageFees_SeparateBill.Tables.Count - 1].Select("fullmonth='" + dsManageFees_SeparateBill.Tables[0].Rows[i]["monthname"].ToString() + "'");
                    divFirstContent.Append("<tr class='odd gradeX'><td class='center'>" + monthtoterm[0][4].ToString() + "</td>");
                }
                else
                    divFirstContent.Append("<tr class='odd gradeX'><td class='center'>" + dsManageFees_SeparateBill.Tables[0].Rows[i]["monthname"].ToString() + "</td>");
                divFirstContent.Append(_Billno_IC);
                divFirstContent.Append(_BillDate_IC);
                divFirstContent.Append(_BillUser_IC);
                divFirstContent.Append("<td>" + strOptions_IC.ToString() + "</td>");
                divFirstContent.Append("</tr>");
    ...
    public static string GetSecondBiMonthContent(DataSet dsManageFees, SqlCommand cmd, string regno, string academicId)
    {
        ...
        if (dsManageFees.Tables[2].Rows.Count > 0 && dsManageFees.Tables[0].Rows.Count > 0)
        {
            ...
            if (cmd.Parameters["@FeesMonth"].Value.ToString() != string.Empty)
            {
                ...
                if (dsManageFees.Tables[3].Rows.Count > 0 && dsManageFees.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int k = 0; k < dsManageFees.Tables[3].Rows.Count; k++)
                    {
                        decimal actualAmount = Convert.ToDecimal(dsManageFees.Tables[3].Rows[k]["actualamount"]);
                        decimal amount = Convert.ToDecimal(dsManageFees.Tables[3].Rows[k]["amount"]);
                        decimal concession = actualAmount - amount;
                        ...
                        feesHeadConcessionAmt += concession.ToString() + "|";
                ...
                dtadv = utl.GetDataTable(@" select sf.*,convert(varchar(10),billdate,103) as datebill,case when Billrefno is null then BillNo else Billrefno end as ReceiptNo  FROM   s_studentinfo info
	  inner join s_studentpromotion promo on info.RegNo=promo.RegNo
             ...
             INNER JOIN f_studentbillmaster sf  
                     ON sf.regno = info.regno  
                        ...
                        AND sf.isactive = 1  and year(sf.billdate)=year(getdate())
      ...
      WHERE  info.regno = '" + regno.ToString() + "' AND info.academicyear = " + HttpContext.Current.Session["AcademicID"] + " and promo.AcademicId= '" + old_academicID + "' order by sf.billID desc");
                ...
                feePOPUPContent.Append("<tr><td style='text-align:center;' ><input id=\"btnSubmit\" type=\"button\" class=\"btn btn-navy\"  value=\"Save & Print\" onclick=\"SaveFeesBiMonthBill('"
                                         + regno + "','"
                                         + academicId + "','"
                                         + feesCatHeadId + "','"
                                         + feesHeadAmt + "','"
    ...
    [WebMethod]
    public static string SaveBiMonthBillDetails(string regNo, string AcademicId, string FeesHeadIds, string FeesAmount, string FeesCatId, string FeesMonthName, string FeestotalAmount,string FeesHeadActualAmt, string FeesHeadConcessionAmt, string BillDate, string userId, string PaymentMode, string CashAmt, string CardAmt, string MonthNum, string FeeType)
    {
        ...
        for (int k = 0; k < MonthNumarr.Length; k++)
        {
            ...
            foreach (DataRow head in rw)
            {
                ...
                //subQuery += "INSERT INTO [dbo].[f_studentbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId], [ActualAmount],[Discount])VALUES(''DummyBill''," + head["feescatheadid"].ToString() + ",''" + FeesMonthNamearr[k].ToString().Trim() + "''," + head["amount"].ToString() + ",''True''," + userId + ")";
                subQuery += "INSERT INTO [dbo].[f_studentbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId],[ActualAmount],[Discount]) " +
            "VALUES(''DummyBill''," + head["feescatheadid"].ToString() + ",''" + FeesMonthNamearr[k].ToString().Trim() + "''," + head["amount"].ToString() + ",''True''," + userId + "," + feeActualAmount[i] + "," + feeConcessionAmount[i] + ")";
    ...
    public static string GetSecondContent(DataSet dsManageSchoolFees, SqlCommand cmd, string regno, string academicId)
    {
        ...
        if (dsManageSchoolFees.Tables[2].Rows.Count > 0 && dsManageSchoolFees.Tables[0].Rows.Count > 0)
        {
            ...
            if (cmd.Parameters["@FeesMonth"].Value.ToString() != string.Empty)
            {
                ...
                decimal schoolConcessionTotal = 0, ICTYpe_schoolConcessionTotal=0;
                ...
                if (dsManageSchoolFees.Tables[3].Rows.Count > 0 && dsManageSchoolFees.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int k = 0; k < dsManageSchoolFees.Tables[3].Rows.Count; k++)
                    {
                       ...
                       //   feePOPUPHeadDetails.Append(@"<td class='tdbrd amt-rgt'>" + Math.Round(Convert.ToDecimal(dsManageSchoolFees.Tables[3].Rows[k]["amount"].ToString().ToUpper())) + "-" + Math.Round(Convert.ToDecimal(dsManageSchoolFees.Tables[3].Rows[k]["actualamount"].ToString().ToUpper())) + "</td></tr>");
                    ...
                    feePOPUPHeadDetails.Append(@"<tr class='tlt-rs'><td height='30' class='tdbrd'>Total </td>
                                                    <td class='tdbrd amt-rgt'>Rs. " + Math.Round(Convert.ToDecimal(sumofactualAmt)) + "</td></tr></table>");
                ...
                string sport_Student = utl.ExecuteASSScalar("select count(*) from s_studentinfo where RegNo = " + dsManageSchoolFees.Tables[2].Rows[0]["RegNo"].ToString() + " and academicyear=" + sport_academicID + " and regno not in(select regno from s_studentcancellation where regno='"+dsManageSchoolFees.Tables[2].Rows[0]["RegNo"].ToString() +"' and academicID='"+ sport_academicID +"' and isactive=1)");
                 SqlConnection sport_conn = new SqlConnection(ConfigurationManager.AppSettings["ASSConnection"].ToString());
                    ...
                    {
                        string ICTYPE_Student = utl.ExecuteScalar("select IsICStudent from s_studentinfo where RegNo = " + dsManageSchoolFees.Tables[2].Rows[0]["RegNo"].ToString() + " and academicID=" + academicId);
                        if (ICTYPE_Student == "Y")
                        {
                            // Utilities utl = new Utilities();
                            ...
                            ICTYpe_cmd = new SqlCommand("sp_ManageFee_SeparateBill");
                            ...
                            if (dataICType != null && dataICType.Tables.Count > 0 && dataICType.Tables[3].Rows.Count > 0)
                            {
                                ...
                                for (int k = 0; k < dataICType.Tables[3].Rows.Count; k++)
                                {
                                    ...
                                    decimal IC_amount = 0;
                                    decimal.TryParse(dataICType.Tables[3].Rows[k]["amount"].ToString(), out IC_amount);

                                    decimal ICTYpe_actualAmt = Convert.ToDecimal(dataICType.Tables[3].Rows[k]["actualamount"]);
                                    decimal ICTYpe_amount = Convert.ToDecimal(dataICType.Tables[3].Rows[k]["amount"]);
                                    decimal ICTYpe_concession = ICTYpe_actualAmt - ICTYpe_amount;
                                    ...
                                    ICTYpe_feesHeadActualAmt += dataICType.Tables[3].Rows[k]["actualamount"].ToString() + "|";
                                    ICTYpe_feesConcessionAmt += ICTYpe_concession + "|";
                                    ...
                                    feePOPUPHeadDetails.Append(@"<td class='tdbrd amt-rgt'>" + Math.Round(Convert.ToDecimal(dataICType.Tables[3].Rows[k]["actualamount"].ToString().ToUpper())) + "</td></tr>");

                                    //ICTYpe_feestotalAmount += Math.Round(Convert.ToDecimal(dataICType.Tables[3].Rows[k]["amount"].ToString().ToUpper()));
                                    ICTYpe_feestotalAmount += Math.Round(IC_amount);
                                    ICTYpe_schoolConcessionTotal += ICTYpe_concession;
                                }
                                feePOPUPHeadDetails.Append(@"</table></div></td></tr>");
                                //feePOPUPHeadDetails.Append(@"<tr class='tlt-rs'><td height='30' class='tdbrd'>Total </td>
                                ...
                                feePOPUPHeadDetails.Append(@"<tr class='tlt-rs'><td height='30' class='tdbrd'>Total </td>
                                                        <td class='tdbrd amt-rgt'>Rs. " + Math.Round(ICTYpe_feestotalAmount) + "</td></tr>");
                ...
                decimal totalConcession = schoolConcessionTotal + ICTYpe_schoolConcessionTotal;

                //if (schoolConcessionTotal > 0 || ICTYpe_schoolConcessionTotal > 0)
                if(totalConcession > 0)
                {
                    feePOPUPHeadDetails.Append(@"<tr class='tlt-rs'>
                                                 <td height='30' class='tdbrd'>Concession Granted</td>
                                                 <td class='tdbrd amt-rgt'>Rs. " + Math.Round(totalConcession) + @"</td>
                                                 </tr>");
                }
                ...
                //feePOPUPHeadDetails.Append(@"</table>");    
                DataTable dtadv = new DataTable();
	  ...
	  inner join f_studentbills d on d.BillId=a.BillId and d.isactive=1 and year(a.billdate)=year(getdate())
      ...
      WHERE  info.regno = '" + dsManageSchoolFees.Tables[2].Rows[0]["RegNo"].ToString() + "' AND info.academicyear = " + HttpContext.Current.Session["AcademicID"] + " and b.AcademicId= '" + old_academicID + "' order by a.billID desc");
                ...
                //if (dataSports != null && dataSports.Tables[0].Rows.Count > 0)
                //{
                //    feePOPUPContent.Append("<tr><td style='text-align:center;' ><input id=\"btnSubmit\" type=\"button\" class=\"btn btn-navy testsport\"  value=\"Save & Print\" onclick=\"SaveFeesBill(\'" + regno + "\',\'" + academicId + "-" + sport_academicID + "\',\'" + feesCatHeadId + "-" + sport_feesCatHeadId + "\',\'" + feesHeadAmt + "-" + sport_feesHeadAmt + "\',\'" + feesCatId + "-" + sport_feesCatId + "\',\'" + feesMonthName + "-" + sport_feesMonthName + "\',\'" + Math.Round(Convert.ToDecimal(feestotalAmount)) + "-" + Math.Round(Convert.ToDecimal(sport_feestotalAmount)) + "\');\" /></td> </tr>");
                //}
                //else
                //{
                //    feePOPUPContent.Append("<tr><td style='text-align:center;' ><input id=\"btnSubmit\" type=\"button\" class=\"btn btn-navy test\"  value=\"Save & Print\" onclick=\"SaveFeesBill(\'" + regno + "\',\'" + academicId + "\',\'" + feesCatHeadId + "\',\'" + feesHeadAmt + "\',\'" + feesCatId + "\',\'" + feesMonthName + "\',\'" + Math.Round(Convert.ToDecimal(feestotalAmount)) + "\');\" /></td> </tr>");
                //}
                ...
                if (dataSports != null && dataSports.Tables.Count > 0 && dataSports.Tables[0].Rows.Count > 0)
                {
                    ...
                    feePOPUPContent.Append("<tr><td style='text-align:center;' ><input id=\"btnSubmit\" type=\"button\" class=\"btn btn-navy testsport\"  value=\"Save & Print\" onclick=\"SaveFeesBill(\'"
                        ...
                        + feesConcessionAmt + "-" + sport_feesConcessionAmt + "\',\'"
                        + feeCategory + "\');\" /></td></tr>");
                    // + feesConcessionAmt + "-" + sport_feesConcessionAmt + "\');\" /></td> </tr>");
                }
                else if (dataICType != null && dataICType.Tables.Count > 0 && dataICType.Tables[0].Rows.Count > 0)
                {
                    feeCategory = "ICType";
                    feePOPUPContent.Append("<tr><td style='text-align:center;' ><input id=\"btnSubmit\" type=\"button\" class=\"btn btn-navy test-ictype\" value=\"Save & Print\" onclick=\"SaveFeesBill(\'"
                        + regno + "\',\'"
                        + academicId + "-" + academicId + "\',\'"
                        + feesCatHeadId + "-" + ICTYpe_feesCatHeadId + "\',\'"
                        + feesHeadAmt + "-" + ICTYpe_feesHeadAmt + "\',\'"
                        + feesCatId + "-" + feesCatId + "\',\'"
                        + feesMonthName + "-" + feesMonthName + "\',\'"
                        + Math.Round(Convert.ToDecimal(feestotalAmount)) + "-" + Math.Round(ICTYpe_feestotalAmount) + "\',\'"
                        + feesHeadActualAmt + "-" + ICTYpe_feesHeadActualAmt + "\',\'"
                        + feesConcessionAmt + "-" + ICTYpe_feesConcessionAmt + "\',\'"
                        + feeCategory + "\');\" /></td></tr>");
                    // + feesConcessionAmt + "-" + ICTYpe_feesConcessionAmt + "\');\" /></td></tr>");
                }
                else
                {
                    feeCategory = "School";
  ...
  /* public static string GetSecondContent(DataSet dsManageSchoolFees, SqlCommand cmd, string regno, string academicId)
    ...
    /*  [WebMethod]
      public static string SaveBillDetails(string regNo, string AcademicId, string FeesHeadIds, string FeesAmount, string FeesCatId, string FeesMonthName, string FeestotalAmount, string BillDate, string userId, string PaymentMode, string CashAmt, string CardAmt, string MonthNum, string FeeType)
      {
          ...
          if (dsSaveBill != null && dsSaveBill.Tables.Count > 0 && dsSaveBill.Tables[0].Rows.Count > 0)
          {
              ...
              string TaxBillNo = "GST/" + schoolabbrivation.ToString().Trim() + "/" + "" + AcademicYear + "/000" + TaxBillcnt.ToString();
    ...
    public static string SaveBillDetails(string regNo, string AcademicId, string FeesHeadIds, string FeesAmount, string FeesCatId, string FeesMonthName, string FeestotalAmount, string FeesHeadActualAmt, string FeesHeadConcessionAmt,string feeCategory, string BillDate, string userId, string PaymentMode, string CashAmt, string CardAmt, string QRAmount, string Remarks, string MonthNum, string FeeType)
    {
        ...
        string[] faConcession = FeesHeadConcessionAmt.Split('-');
        ...
        string[] sport_feeActualamt = null;

        if (faActual.Length > 1)
        {
            sport_feeActualamt = faActual[1].Split('|');
        }

        string[] sport_feeConcessionamt = null;

        if (faConcession.Length > 1)
        {
            sport_feeConcessionamt = faConcession[1].Split('|');
        }
        ...
        //string[] fActualta = FeesHeadActualAmt.Split('-');
        //string feeactualamt = fActualta[0].ToString();
        //string sport_feeActualamt = string.Empty;
        //if (fActualta.Length > 1)
        //{
        //    sport_feeActualamt = fActualta[1].ToString();
        //}
        ////
        //string[] fConcessionta = FeesHeadConcessionAmt.Split('-');
        //string feeConcessiomamt = fConcessionta[0].ToString();
        //string sport_feeConcessionamt = string.Empty;
        //if (fConcessionta.Length > 1)
        //{
        //    sport_feeConcessionamt = fConcessionta[1].ToString();
        //}
        ...
        if (sport_feeHead.Length > 0)
        {
            foreach (string s_head in sport_feeHead)
            {
                ...
                {
                    //subAssQuery += "INSERT INTO [dbo].[f_studentbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId])
                    //VALUES(''DummyBill'',"  + s_head + ",''" + sport_feemonth + "''," + sport_feeAmount[i] + ",''True''," + userId + ")";
                    subAssQuery += "INSERT INTO [dbo].[f_studentbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId],[ActualAmount],[Discount]) " +
                    "VALUES(''DummyBill''," + s_head + ",''" + sport_feemonth + "''," + sport_feeAmount[i] + ",''True''," + userId + "," + feeActualAmount[i] + "," + feeConcessionAmount[i] + ")";
            ...
            if (sport_feecatid != "" && sport_feemonth != "" && subAssQuery != "")
            {
                ...
                //  query = "[SP_InsertFeesBill] '0000'," + academic + ",'" + FeesCatId + "',"                  + regNo + ",'" + feemonth + "'," + feetotalamt + ",'" + formatBillDate + "'," + userId + ",'" + subQuery + "'," + PaymentMode + ",'" + cashamt + "','" + cardamt + "','" + qramt + "','" + Remarks + "'";
        ...
        foreach (string head in feeHead)
        {
            if (head != null && head != "")
            {
                //subQuery += "INSERT INTO [dbo].[f_studentbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId])
                //VALUES(''DummyBill''," + head + ",''" + feemonth + "''," + feeAmount[i] + ",''True''," + userId + ")";
                // subQuery += "INSERT INTO [dbo].[f_studentbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId],[ActualAmount],[Discount])
                // VALUES(''DummyBill''," + head + ",''" + feemonth + "''," + feeAmount[i] + ",''True''," + userId + ")";
                subQuery += "INSERT INTO [dbo].[f_studentbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId],[ActualAmount],[Discount]) " +
        ...
        //IC Bill Save
        string subICQuery = string.Empty;
        if (feeCategory == "ICType")
        {
            if (sport_feeHead.Length > 0)
            {
               i = 0;
                foreach (string s_head in sport_feeHead)
                {
                    if (s_head != "" && s_head != null)
                    {
                        subICQuery += "INSERT INTO [dbo].[f_studentbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId],[ActualAmount],[Discount]) " +
                                        "VALUES(''DummyBill''," + s_head + ",''" + sport_feemonth + "''," + sport_feeAmount[i] + ",''True''," + userId + "," + sport_feeActualamt[i] + "," + sport_feeConcessionamt[i] + ")";
                        i++;
                    }
                ...
                if (sport_feecatid != "" && sport_feemonth != "" && subICQuery != "")
                {
                    ass_query = "[SP_InsertFeesBill] '0002'," + sport_academic + ",'" + sport_feecatid + "'," + regNo + ",'" + sport_feemonth + "'," + sport_feetotalamt + ",'" + formatBillDate + "'," + userId + ",'" + subICQuery + "'," + PaymentMode + ",'" + sport_cashamt + "','" + sport_cardamt + "','" + sport_qramt + "','" + Remarks + "'";
                    dsSportSaveBill = utl.GetDataset(ass_query);
                }
        ...
        if (dsSaveBill != null && dsSaveBill.Tables.Count > 0 && dsSaveBill.Tables[0].Rows.Count > 0)
        {
            ...
            foreach (string head in feeHead)
            {
                if (head != null && head != "")
                {
                    ...
                    if (dttax.Rows.Count > 0)
                    {
                        ...
                        taxQuery = "insert into f_studenttaxbills(TaxBillID,FeesCatId,TaxID,TaxPercent,TaxAmount,isactive,userid)values('" + taxBillID + "','" + head + "','" + dttax.Rows[0]["TaxID"].ToString() + "','" + dttax.Rows[0]["Percentage"].ToString() + "','" + taxvalue + "','true','" + userId + "')";
                        utl.ExecuteQuery(taxQuery);
                        utl.ExecuteQuery("update f_studenttaxbills set isactive=1 where TaxBillID='" + taxBillID.ToString() + "'");
                    }
            ...
            utl.ExecuteQuery("update f_studenttaxbillmaster set isactive=1 where BillID='" + dsSaveBill.Tables[0].Rows[0][0].ToString() + "'");
