...
public partial class Students_HostelFees : System.Web.UI.Page
{
    Utilities utl = null;
    ...
    public static string BindAcademicYearMonth(string regNo, string academicId, string editPrm, string delPrm)
    {
        ...
        if (Isactive == "True")
        {
           cmd = new SqlCommand("sp_ManageHostelFee");
        }
        else
        {
           cmd = new SqlCommand("sp_ManageOldHostelFee");
        }
    ...
    public static string GetFirstContent(DataSet dsManageFees, SqlCommand cmd, string regno, string academicId, string editPrm, string delPrm)
    {
        ...
        if (dsManageFees.Tables[0].Rows.Count > 0)
        {
            ...
            for (int i = 0; i < dsManageFees.Tables[0].Rows.Count; i++)
            {
                ...
                if (dsManageFees.Tables[0].Rows[i]["monthnum"].ToString() == cmd.Parameters["@FeesMonth"].Value.ToString() && cmd.Parameters["@FeesType"].Value.ToString().Trim().ToUpper() == "HOSTEL")
            ...
            if (cmd.Parameters["@FeesMonth"].Value.ToString() != string.Empty)
            {
                ...
                feePOPUPStudentDetails.Append(@"<table width='700px' border='0' cellpadding='3'  cellspacing='0' class='popup-form' >
                                           <tr>
	                                          ...
	                                          <td width='16' >:</td>
	                                          <td width='245' ><label for='textfield2'>" + dsManageFees.Tables[2].Rows[0]["regno"].ToString() + "</label> </td>");
                ...
                feePOPUPStudentDetails.Append(@"<tr><td width='104' height='30' >Name</td>
	                                          <td width='12' >:</td>
	                                          <td width='258' ><label for='textfield2'>" + dsManageFees.Tables[2].Rows[0]["stname"].ToString().ToUpper() + "</label></td>");
                DataRow[] monthtoterm = dsManageFees.Tables[dsManageFees.Tables.Count - 1].Select("fullmonth='" + PaidMonthName + "'");
                feePOPUPStudentDetails.Append(@"<td width='105' >Term </td>
	                                      <td width='16' >:</td>
	                                      <td width='245' ><label for='textfield2'>" + monthtoterm[0][2] + "</label></td></tr>");
                ...
                if (dsManageFees.Tables[3].Rows.Count > 0 && dsManageFees.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int k = 0; k < dsManageFees.Tables[3].Rows.Count; k++)
                    {
                        ...
                        feePOPUPHeadDetails.Append(@"<td class='tdbrd'>" + dsManageFees.Tables[3].Rows[k]["amount"].ToString().ToUpper() + "</td></tr>");
    ...
    public static string SaveBillDetails(string regNo, string AcademicId, string FeesHeadIds, string FeesAmount, string FeesCatId, string FeesMonthName, string FeestotalAmount, string BillDate, string userId, string PaymentMode)
    {
        ...
        {
            subQuery += "INSERT INTO [dbo].[f_hostelbills]([BillId],[FeesCatHeadId],[BillMonth],[Amount],[IsActive],[UserId])VALUES(''DummyBill''," + head + ",''" + FeesMonthName + "''," + feeAmount[i] + ",''True''," + userId + ")";
            i++;
        }
        ...
        string query = "[SP_InsertHostelFeesBill] '0000'," + AcademicId + "," + FeesCatId + "," + regNo + ",'" + FeesMonthName + "'," + FeestotalAmount + ",'" + formatBillDate + "'," + userId + ",'" + subQuery + "'," + PaymentMode;
    ...
    public static string ViewBillDetails(string billId)
    {
        ...
        if (Isactive == "True")
        {
           query = "[sp_GetHostelBill] " + billId + "";
        }
        else
        {
            query = "[sp_GetOldHostelBill] " + billId + ",'" + HttpContext.Current.Session["AcademicID"] + "'";
        }
    ...
    public static void PrintBill(string billId)
    {
        try
        {
            ...
            if (dsPrint.Tables.Count > 1)
            {
                ...
                str.Append("AMALORPAVAM LOURDS ACADEMY \r\n  LOURDS CAMPUS, PUDUCHERRY - 605 001. \r\n");
                ...
                fd.PrintPage += (s, args) =>
                {
                    ...
                    if (dsPrint.Tables[0].Rows.Count > 0)
                    {
                        ...
                        args.Graphics.DrawLine(new Pen(Color.Black, 2), 0, 40, 500, 40);
                        ...
                        args.Graphics.DrawString(" Term 	: 	" + dsPrint.Tables[0].Rows[0]["BillMonth"].ToString(), new System.Drawing.Font("ARIAL", 9, FontStyle.Regular), Brushes.Black, new Rectangle(120, 95, 120, 50), stringAlignRight);
                        ...
                        args.Graphics.DrawString("Cashier : " + dsPrint.Tables[0].Rows[0]["staffshortname"].ToString(), new System.Drawing.Font("ARIAL", 9, FontStyle.Regular), Brushes.Black, new Rectangle(0, yPos + 80, 250, 50), stringAlignLeft);
    ...
    {
        Utilities utl = new Utilities();
        string query = "[SP_DeleteHostelBill] " + billId + "";
