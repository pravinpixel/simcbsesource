...
//setup DatePicker
function setAlternateDatePicker() {
    $("#datepicker").datepicker({
        showOn: "button",
        buttonImage: "img/calendar.gif",
...
function setAltDate() {
    var d = $.datepicker.parseDate('dd/mm/yy', $("#datepicker").val());
    d.setFullYear(d.getFullYear() + 60);
    setDateToAltText(d);
}
function setDateToAltText(value) {
    var year = value.getFullYear()
    var month = value.getMonth();
    var day = value.getDate();
    var full = $.datepicker.formatDate('dd/mm/yy', new Date(year, month, day));
    $("[id*=txtDOR]").val(full);
}
...
function setupLeftMenu() {
    $(".section-menu")
	  ...
	  $(".john-accord")
		...
		$('.john-accord .johnmenu').css('height','auto');	
